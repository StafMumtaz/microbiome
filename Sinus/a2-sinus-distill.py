...
# Inputs → Outputs
PAIRS = [
    ("a1-main-data.csv", "a3-sinus.csv"),
    ("a1b-main-no-aggregate.csv", "a3b-sinus-no-aggregate.csv"),
]
...
def filter_and_sort(in_csv: str, out_csv: str) -> None:
    df = pd.read_csv(in_csv)
    first_col = df.columns[0]

    # Filter: first column starts with 'S' (after normalization)
    norm = _normalize(df[first_col])
    keep_mask = norm.str.startswith("S")
    ...
    print(f"Wrote {out_csv}. Removed {removed_n} rows not starting with 'S' in {first_col} from {in_csv}.")

def main():
    for in_csv, out_csv in PAIRS:
        filter_and_sort(in_csv, out_csv)
